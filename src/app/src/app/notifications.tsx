...
function Toggle({ on, onToggle, c }: { on: boolean; onToggle: () => void; c: { green: string; toggleOff: string } }) {
  return (
    <Pressable
...
export default function NotificationsScreen() {
  ...
  return (
    <ScrollView
      ...
      <Text style={[styles.pageSub, { color: c.t3 }]}>Gentle nudges, not noise.</Text>

      <Text style={[styles.sectionHeader, { color: c.t3 }]}>Event Reminders</Text>
      <View style={[styles.card, { backgroundColor: c.surface, borderColor: c.border }]}>
        ...
        <View style={[styles.custRow, { borderBottomWidth: 0, borderBottomColor: c.border }]}>
          <Text style={[styles.custLabel, { color: c.t1 }]}>All-day events</Text>
      ...
      <View style={[styles.card, { backgroundColor: c.surface, borderColor: c.border }]}>
        <View style={[styles.custRow, { borderBottomColor: c.border }]}>
          <Text style={[styles.custLabel, { color: c.t1 }]}>Morning summary</Text>
          <Toggle on={morningSummary} onToggle={() => setMorningSummary(!morningSummary)} c={c} />
        </View>
        <View style={[styles.custRow, { borderBottomColor: c.border }]}>
          <Text style={[styles.custLabel, { color: c.t1 }]}>Overdue tasks</Text>
          <Toggle on={overdueTasks} onToggle={() => setOverdueTasks(!overdueTasks)} c={c} />
        </View>
        <View style={[styles.custRow, { borderBottomWidth: 0, borderBottomColor: c.border }]}>
          <Text style={[styles.custLabel, { color: c.t1 }]}>Streak updates</Text>
          <Toggle on={streakUpdates} onToggle={() => setStreakUpdates(!streakUpdates)} c={c} />
        </View>
      </View>

      <Text style={[styles.sectionHeader, { color: c.t3 }]}>Preview</Text>
      {[
        { icon: '⚡', title: 'Calendar App', text: 'Design sync in 5 min — you have 1 prep task unfinished.', time: 'Just now' },
        { icon: '📋', title: 'Morning Summary', text: '4 events, 3 tasks today. First up: standup at 9.', time: '8:00 AM' },
      ].map((n) => (
        <View key={n.title} style={[styles.notif, { backgroundColor: c.surface, borderColor: c.border }]}>
...
const styles = StyleSheet.create({
  ...
  sectionHeader: {
    ...
    letterSpacing: 0.5, marginTop: 20, marginBottom: 8, paddingHorizontal: 4,
  },
  ...
  notif: {
    flexDirection: 'row', gap: 10, borderWidth: 1, borderRadius: 14, padding: 14, marginBottom: 8,
  },
